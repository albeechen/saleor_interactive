...
def get_user_wishlist(
    user: User, Wishlist=Wishlist.objects.all(), auto_create=False
) -> Tuple[Optional[Wishlist], bool]:
    """Return an active checkout for given user or None if no auto create.
...
def get_or_create_wishlist_from_request(
    ...
    """Fetch checkout from database or create a new one based on cookie."""
    if request.user.is_authenticated:
        return get_user_checkout(request.user, wishlist_queryset, auto_create=True)[0]
    token = request.get_signed_cookie(COOKIE_NAME, default=None)

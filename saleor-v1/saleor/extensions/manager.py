...
class ExtensionsManager(PaymentInterface):
    ...
    def calculate_wishlist_total(
        self, wishlist: "Wishlist", discounts: List["DiscountInfo"]
    ) -> TaxedMoney:
        total = wishlist.get_total(discounts)
        default_value = quantize_price(
            TaxedMoney(net=total, gross=total), total.currency
        )
        return self.__run_method_on_plugins(
            "calculate_wishlist_total", default_value, wishlist, discounts
        )
    ...
    def calculate_wishlist_subtotal(
        self, wishlist: "Wishlist", discounts: List["DiscountInfo"]
    ) -> TaxedMoney:
        subtotal = wishlist.get_subtotal(discounts)
        default_value = quantize_price(
            TaxedMoney(net=subtotal, gross=subtotal), subtotal.currency
        )
        return self.__run_method_on_plugins(
            "calculate_wishlist_subtotal", default_value, wishlist, discounts
        )
    ...
    def calculate_wishlist_shipping(
        self, wishlist: "Wishlist", discounts: List["DiscountInfo"]
    ) -> TaxedMoney:
        total = wishlist.get_shipping_price()
        total = TaxedMoney(net=total, gross=total)
        default_value = quantize_price(total, total.currency)
        return self.__run_method_on_plugins(
            "calculate_wishlist_shipping", default_value, wishlist, discounts
        )
    ...
    def calculate_wishlist_line_total(
        self, wishlist_line: "WishlistLine", discounts: List["DiscountInfo"]
    ):
        total = wishlist_line.get_total(discounts)
        default_value = quantize_price(
            TaxedMoney(net=total, gross=total), total.currency
        )
        return self.__run_method_on_plugins(
            "calculate_wishlist_line_total", default_value, wishlist_line, discounts
        )
    ...
    def preprocess_order_creation(
        ...
        default_value = None
        return self.__run_method_on_plugins(
            "preprocess_order_creation", default_value, wishlist, discounts
        )

...
class WishlistQueryset(models.QuerySet):
    """A specialized queryset for dealing with wishlists."""
...
class Wishlist(ModelWithMetadata):
    ...
    gift_cards = models.ManyToManyField(GiftCard, blank=True, related_name="wishlists")
    ...
    def __repr__(self):
        return "Wishlist(quantity=%s)" % (self.quantity,)
    ...
    def get_subtotal(self, discounts=None):
        """Return the total cost of the wishlist prior to shipping."""
    ...
    def get_total_gift_cards_balance(self):
        """Return the total balance of the gift cards assigned to the wishlist."""
...
class WishlistLine(models.Model):
    """A single wishlist line.

    Multiple lines in the same wishlist can refer to the same product variant if
    their `data` field is different.
    """

    wishlist = models.ForeignKey(
        Wishlist, related_name="lines", on_delete=models.CASCADE
    )
    ...
    def __eq__(self, other):
        if not isinstance(other, WishlistLine):
    ...
    def __repr__(self):
        return "WishlistLine(variant=%r, quantity=%r)" % (self.variant, self.quantity)

...
def test_set_unpublished_homepage_collection(admin_client, collection, site_settings):
...
def test_collection_create_with_background_image(monkeypatch, admin_client, media_root):
    mock_create_thumbnails = Mock(return_value=None)
    monkeypatch.setattr(
        (
            "saleor.dashboard.collection.forms."
    ...
    data = {"name": "Name", "background_image": image}
    url = reverse("dashboard:collection-add")

    response = admin_client.post(url, data)
    assert response.status_code == 302
    mock_create_thumbnails.assert_called_once_with(Collection.objects.last().pk)


def test_collection_update_view(monkeypatch, admin_client, collection, product):
    mock_create_thumbnails = Mock(return_value=None)
    monkeypatch.setattr(
        (
            "saleor.dashboard.collection.forms."
...
def test_collection_update_background_image(
    ...
    data = {"name": "New name", "background_image": image}
    response = admin_client.post(url, data)
    assert response.status_code == 302
